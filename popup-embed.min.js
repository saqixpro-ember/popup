(function(){
  ...
  async function fetchKillList() {
    ...
    try {
      ...
      const popupUrlCol = hdr.indexOf("popup url");
      ...
      for (let i = startIdx; i < rows.length; i++) {
        ...
        const popupUrl = (hasHeader ? r[popupUrlCol] : r[7]) || "";
        const normalizedUrl = url.trim();
        const normalizedEnabled = normalizeBool(enabled);
        const normalizedPopupUrl = popupUrl.trim();
        entries.push({ url: normalizedUrl, enabled: normalizedEnabled, popupUrl: normalizedPopupUrl });
      }
  ...
  function shouldBlock(entries, host) {
    if (!entries || !entries.length) return { block: false, popupUrl: null };
    for (const e of entries) {
      if (patternMatchesHost(e.url, host)) {
        ...
        if (!e.enabled) {
          ...
          return { block: true, popupUrl: null };
        } else {
          console.log("[PopupEmbed] ALLOWING popup for:", e.url);
          return { block: false, popupUrl: e.popupUrl };
        }
      }
    }
    return { block: false, popupUrl: null };
  }
  ...
  async function boot() {
    try {
      ...
      const result = shouldBlock(list, HOST);
      if (result.block) {
        return;
      }
      if (result.popupUrl) {
        window.PopupEmbed.open({ url: result.popupUrl });
      }
    } catch (error) {
      console.log("[PopupEmbed] Error in boot process:", error);
    }
  }
